...
export default function ArenaWorkspace({
  ...
  const { compileAndExecute: wasmCompileAndExecute, isCompiling, isExecuting: wasmIsExecuting } = useWasmExecution();
  ...
  const run = async () => {
    ...
    if ((language === "cpp" || language === "go") && wasmCompileAndExecute) {
      try {
